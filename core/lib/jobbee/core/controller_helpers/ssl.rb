  ...
  module Core
    module ControllerHelpers
      module SSL
        ...
        included do
          before_filter :force_non_ssl_redirect, :if => Proc.new { Jobbee::Config[:redirect_https_to_http] }
          ...
          def self.ssl_supported?
            return Jobbee::Config[:allow_ssl_in_production] if Rails.env.production?
            return Jobbee::Config[:allow_ssl_in_staging] if Rails.env.staging?
            return Jobbee::Config[:allow_ssl_in_development_and_test] if (Rails.env.development? or Rails.env.test?)
          end
            ...
            def force_non_ssl_redirect(host = nil)
              if request.ssl? && !ssl_allowed?
                if request.get?
                  ...
                  render :text => Jobbee.t(:change_protocol, :scope => :ssl), :status => :upgrade_required
                end
              end
            end
        end
